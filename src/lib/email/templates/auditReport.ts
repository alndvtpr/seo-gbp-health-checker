import type { AuditEmailData } from '@/app/actions/send-audit-report'

export function getGradeColor(grade: string): { hex: string; bg: string } {
...
export function generateAuditEmailHtml(data: AuditEmailData): string {
  const {
    businessName,
    location,
    totalScore,
...
export function generateAuditEmailText(data: AuditEmailData): string {
  const {
    businessName,
    location,
    totalScore,
