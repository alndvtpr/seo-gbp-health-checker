...
export default function ServicesPage() {
  return (
    <div className="pt-32 pb-24 px-6 md:px-16 max-w-7xl mx-auto relative z-20 space-y-20">
        ...
        {services.map((s) => (
          ...
          >
            <div>
              ...
              <h2 className="font-heading text-2xl font-bold text-on-surface mb-3">{s.title}</h2>
              <p className="font-sans text-sm text-on-surface/70 leading-relaxed mb-8">{s.desc}</p>
            ...
            <Link
              href="/contact"
              className="inline-flex items-center justify-center w-full py-3.5 rounded-xl bg-white/5 hover:bg-primary-container hover:text-on-primary-container font-heading text-xs font-bold uppercase tracking-widest text-on-surface transition-all"
            >
      ...
      {/* Bottom CTA */}
      <div className="p-10 rounded-3xl bg-gradient-to-r from-primary-container/20 via-[#181a1b] to-[#181a1b] border border-primary-container/30 text-center space-y-6">
        <h2 className="font-heading text-3xl font-extrabold text-on-surface">Need a Custom Strategy?</h2>
        <p className="font-sans text-sm text-on-surface/70 max-w-xl mx-auto">
          Every site has unique requirements. Let&apos;s assemble a bespoke audit and execution plan tailored to your business goals.
        </p>
        <Link
          href="/contact"
          className="inline-flex items-center gap-2 bg-primary-container text-on-primary-container font-heading text-xs font-bold uppercase tracking-widest px-8 py-4 rounded-full shadow-[0_0_30px_rgba(230,126,34,0.5)] hover:scale-105 transition-all"
        >
