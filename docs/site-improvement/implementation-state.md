...
## Post-Phase 20 architecture reconciliation evidence (2026-08-30)

- Separate-work context: after the owner-authorized `120979c` Phase 13–20 checkpoint, GitHub PRs added `1be8de2` (shared navigation/static sitemap registry), `6c38752` (homepage static-section extraction) and `d15c592` (shared site identity and breadcrumb schema). Git ancestry proves this is a linear continuation of `120979c`, not a competing or overwritten history.
- Safe synchronization: under D-035, the clean local `main` fast-forwarded to `origin/main` at `d15c592` with `git pull --ff-only`; no merge commit, reset, rebase, force operation, conflict resolution or unrelated-file overwrite occurred.
- Initial validation finding: TypeScript, lint, four static suites, the isolated build and rendered crawl passed. Accessibility reported 11/12 because its exact multiline source matcher assumed LF after a CRLF checkout. Content reported 8/10 because unchanged homepage prices and CTA markup moved from `page.tsx` to `HomeSections.tsx` while the source test still read only the route file.
- Bounded repair under D-036: `scripts/verify-accessibility.ts` now normalizes CRLF/CR to LF before source assertions. `scripts/verify-content.ts` now evaluates the composed homepage source (`page.tsx` plus `HomeSections.tsx`). No production source, public copy, route, price, schema or behavior changed in this repair.
- Validation actually run after repair: direct TypeScript passed; full lint exits 0 with zero errors/12 warnings; SEO/privacy/security 17/17, accessibility 12/12, performance/evidence 7/7, responsive 9/9, theme 10/10 and content/facts 10/10 pass (65/65 total); `git diff --check` passes.
- Isolated build and crawl: the production build generated all 29 route units with `postgresql://reconcile:reconcile@127.0.0.1:1/reconcile`; the expected connection refusal prevented database access. All 17 canonical routes returned HTTP 200 with exact self-canonicals and one H1; 34 JSON-LD scripts parsed; 18 discovered local route targets and 30 discovered local asset paths passed; sitemap, feed/media types, security headers and rendered protected homepage contracts passed.
- Preservation: P-007, P-008 and P-009 remain unresolved; the exact approved About narrative/training wording, all approved PHP/USD prices, the ₱500/hour rate, 20–25 monthly hours, capacity/scope wording, canonical routes, schema relationships, accessibility behavior, evidence classifications, responsive behavior, theme parity and owner-supplied personal facts remain intact. `fact-inventory.md` remains unchanged because no factual, commercial, privacy, credential or positioning evidence changed.
